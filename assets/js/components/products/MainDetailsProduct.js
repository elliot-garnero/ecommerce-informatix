...
class MainDetailsProduct extends Component {
  ...
  componentDidMount() {
    ...
    axios.get('http://localhost:8000/api/products/' + id).then((res) => {
      ...
      axios
        .get('http://localhost:8000/api/productsName/' + this.state.name)
        .then((res) => {
          this.setState({
            productName: res.data,
          });
          axios
            .post('http://localhost:8000/product/reviews/' + id)
            .then((res) => {
              this.setState({
                reviews: res.data,
              });
              ...
              reviews.length > 0 &&
                reviews.map((review) => {
                  ...
                  axios
                    .post(
                      'http://localhost:8000/product/reviewsName/' +
                        this.state.idUserReview
                    )
                    .then((res) => {
                      this.setState({
                        idUserOne: res.data,
  ...
  async selectChange(event) {
    ...
    await axios
      .get('http://localhost:8000/api/products/' + this.state.idProd)
      .then((res) => {
        this.setState({
          data: res.data,
          name: res.data.name,
  ...
  render() {
    ...
    let selectColor =
      ...
      productName.map((item, i) => {
        return (
          <option key={i} value={item.idProduct}>
            {item.color}
          </option>
        );
      }, this);
    ...
    return (
      <div className="container-mb">
            ...
            <div>
              ...
              {data.color !== null && (
                <span className="tagged_as">
                  <strong>Couleur :</strong> {data.color} &nbsp;
                  <select
              ...
              {data.size !== null && (
                <span className="tagged_as">
                  <strong>Taille :</strong> {data.size} cm &nbsp;
                  <select
                    value={this.state.idProd}
                    onChange={this.selectChange}
                    name="size"
              ...
              <p>
                <strong>Poids :</strong> {data.weight} g<br></br>
              </p>
            </div>
            ...
            {data.stock > 0 && (
              <div>
                <p className="text-success">
                  <strong>En stock :</strong> {data.stock}
                </p>
                <div className="m-bot15">
                  ...
                  <strong>
                    <h3 className="text-warning">Prix : {data.price} €</h3>
                  </strong>
                  <br></br>
                </div>
                <button
              ...
              <div>
                <div className="m-bot15">
                  ...
                  <h5 className="text-danger">
                    <strong>
                      <em>Produit indisponible pour le moment</em>
                    </strong>
                  </h5>
                  <br></br>
                  <strong>
        ...
        <div className="">
          <h4>{count} Avis</h4>
          {reviews.length == 0 && (
            <div className="pb-5">Aucun avis n'a été laissé sur ce produit</div>
          )}
          {reviews.length > 0 && (
            <div className="pb-4">
              {reviews.map((review, i) => (
                <div key={i} id={review.id} className="border p-3">
                  ...
                  <div>
                    <p>"{review.content}"</p>
                    <p>
                      <em>écrit par {review.username}</em>
                    </p>
                  </div>
                </div>
              ))}
        ...
        <div className="row">
          <div className="container-mb">
            <SliderSuggestion dataFromParent={brand} />
          </div>
        </div>
      </div>
    );
